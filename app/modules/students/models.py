...
class Student(Base):
    ...
    __table__ = Table(
        ...
        UniqueConstraint(
            "tenant_id", "student_number", name="uq_sms_students_tenant_student_number"
        ),
        UniqueConstraint("tenant_id", "id", name="uq_sms_students_tenant_id_id"),
        CheckConstraint(
            "current_status IN ('APPLICANT', 'ADMITTED', 'ACTIVE', 'INACTIVE', 'WITHDRAWN', 'COMPLETED')",
            name="ck_sms_students_current_status",
        ),
        CheckConstraint(
            "source_type IN ('MANUAL', 'IMPORT', 'MIGRATION', 'API', 'SYSTEM', 'AI_ASSISTED')",
            name="ck_sms_students_source_type",
        ),
        ForeignKeyConstraint(
            ["tenant_id"], ["sms_tenants.id"], name="fk_sms_students_tenant", ondelete="RESTRICT"
        ),
        ForeignKeyConstraint(
            ["created_by"], ["sms_users.id"], name="fk_sms_students_created_by", ondelete="RESTRICT"
        ),
        ForeignKeyConstraint(
            ["updated_by"], ["sms_users.id"], name="fk_sms_students_updated_by", ondelete="SET NULL"
        ),
        ...
        Index(
            "ix_sms_students_mobile",
            "tenant_id",
            "student_mobile",
            postgresql_where=text("student_mobile IS NOT NULL"),
        ),
        Index(
            "ix_sms_students_email_lower",
            "tenant_id",
            text("lower(student_email)"),
            postgresql_where=text("student_email IS NOT NULL"),
        ),
    )
...
class StudentAlias(Base):
    ...
    __table__ = Table(
        ...
        CheckConstraint(
            ...
            name="ck_sms_student_aliases_validity",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "student_id"],
            ["sms_students.tenant_id", "sms_students.id"],
            name="fk_sms_student_aliases_student_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "branch_id"],
            ["sms_branches.tenant_id", "sms_branches.id"],
            name="fk_sms_student_aliases_branch_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["created_by"],
            ["sms_users.id"],
            name="fk_sms_student_aliases_created_by",
            ondelete="RESTRICT",
        ),
        Index(
            "uq_sms_student_aliases_tenant_scope",
            "tenant_id",
            "alias_type",
            "alias_value",
            unique=True,
        ...
        Index(
            "uq_sms_student_aliases_branch_scope",
            "tenant_id",
            "branch_id",
            "alias_type",
            "alias_value",
...
class Enrollment(Base):
    ...
    __table__ = Table(
        ...
        CheckConstraint(
            "status IN ('DRAFT', 'ACTIVE', 'PROMOTED', 'TRANSFERRED', 'WITHDRAWN', 'COMPLETED', 'CLOSED')",
            name="ck_sms_enrollments_status",
        ),
        CheckConstraint(
            "source_type IN ('MANUAL', 'IMPORT', 'MIGRATION', 'API', 'SYSTEM')",
            name="ck_sms_enrollments_source_type",
        ),
        CheckConstraint(
            "ending_date IS NULL OR ending_date >= joining_date", name="ck_sms_enrollments_dates"
        ),
        CheckConstraint(
            "transfer_source_enrollment_id IS NULL OR transfer_source_enrollment_id <> id",
            name="ck_sms_enrollments_transfer_self",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "student_id"],
            ["sms_students.tenant_id", "sms_students.id"],
            name="fk_sms_enrollments_student_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "branch_id"],
            ["sms_branches.tenant_id", "sms_branches.id"],
            name="fk_sms_enrollments_branch_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "academic_year_id"],
            ["sms_academic_years.tenant_id", "sms_academic_years.id"],
            name="fk_sms_enrollments_academic_year_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "programme_id"],
            ["sms_academic_programmes.tenant_id", "sms_academic_programmes.id"],
            name="fk_sms_enrollments_programme_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "branch_id", "batch_id"],
            ["sms_batches.tenant_id", "sms_batches.branch_id", "sms_batches.id"],
            name="fk_sms_enrollments_batch_hierarchy",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "branch_id", "batch_id", "section_id"],
            [
                "sms_sections.tenant_id",
                "sms_sections.branch_id",
                "sms_sections.batch_id",
                "sms_sections.id",
            ],
            name="fk_sms_enrollments_section_hierarchy",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "transfer_source_enrollment_id"],
            ["sms_enrollments.tenant_id", "sms_enrollments.id"],
            name="fk_sms_enrollments_transfer_source",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["created_by"],
            ["sms_users.id"],
            name="fk_sms_enrollments_created_by",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["updated_by"],
            ["sms_users.id"],
            name="fk_sms_enrollments_updated_by",
            ondelete="SET NULL",
        ),
        Index(
            "uq_sms_enrollments_current_active",
            "student_id",
            unique=True,
            postgresql_where=text("is_current = true AND status = 'ACTIVE'"),
        ),
        Index(
            "uq_sms_enrollments_branch_admission_number",
            "tenant_id",
            "branch_id",
            "admission_number",
            unique=True,
        ...
        Index(
            "uq_sms_enrollments_branch_year_roll_number",
            "tenant_id",
            "branch_id",
            "academic_year_id",
            "roll_number",
        ...
        Index(
            "ix_sms_enrollments_scope_status",
            "tenant_id",
            "branch_id",
            "academic_year_id",
            "status",
...
class Guardian(Base):
    ...
    __table__ = Table(
        ...
        CheckConstraint(
            ...
            name="ck_sms_guardians_verification_status",
        ),
        CheckConstraint(
            "status IN ('ACTIVE', 'BLOCKED', 'INACTIVE')", name="ck_sms_guardians_status"
        ),
        CheckConstraint(
            "verification_status <> 'VERIFIED' OR verified_at IS NOT NULL",
            name="ck_sms_guardians_verified_at",
        ),
        ForeignKeyConstraint(
            ["tenant_id"], ["sms_tenants.id"], name="fk_sms_guardians_tenant", ondelete="RESTRICT"
        ),
        ForeignKeyConstraint(
            ["tenant_id", "portal_user_id"],
            ["sms_users.tenant_id", "sms_users.id"],
            name="fk_sms_guardians_portal_user_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["created_by"],
            ["sms_users.id"],
            name="fk_sms_guardians_created_by",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["updated_by"],
            ["sms_users.id"],
            name="fk_sms_guardians_updated_by",
            ondelete="SET NULL",
        ),
        Index(
            "uq_sms_guardians_tenant_portal_user",
            "tenant_id",
            "portal_user_id",
            unique=True,
            postgresql_where=text("portal_user_id IS NOT NULL"),
        ),
        Index("ix_sms_guardians_tenant_mobile", "tenant_id", "mobile"),
        Index(
            "ix_sms_guardians_portal_user",
            "portal_user_id",
            postgresql_where=text("portal_user_id IS NOT NULL"),
        ),
...
class StudentGuardianLink(Base):
    ...
    __table__ = Table(
        ...
        timestamp("updated_at", nullable=False, default_now=True),
        CheckConstraint(
            "relationship_type IN ('FATHER', 'MOTHER', 'LEGAL_GUARDIAN', 'RELATIVE', 'SPONSOR', 'OTHER')",
            name="ck_sms_student_guardian_links_relationship",
        ),
        CheckConstraint(
            "verification_status IN ('PENDING', 'VERIFIED', 'REJECTED', 'REVOKED')",
            name="ck_sms_student_guardian_links_verification_status",
        ),
        CheckConstraint(
            "status IN ('ACTIVE', 'SUSPENDED', 'REVOKED')",
            name="ck_sms_student_guardian_links_status",
        ),
        CheckConstraint(
            "effective_until IS NULL OR effective_until > effective_from",
            name="ck_sms_student_guardian_links_validity",
        ),
        CheckConstraint(
            "verification_status <> 'VERIFIED' OR verified_at IS NOT NULL",
            name="ck_sms_student_guardian_links_verified_at",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "student_id"],
            ["sms_students.tenant_id", "sms_students.id"],
            name="fk_sms_student_guardian_links_student_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["tenant_id", "guardian_id"],
            ["sms_guardians.tenant_id", "sms_guardians.id"],
            name="fk_sms_student_guardian_links_guardian_tenant",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["verified_by"],
            ["sms_users.id"],
            name="fk_sms_student_guardian_links_verified_by",
            ondelete="SET NULL",
        ),
        ForeignKeyConstraint(
            ["created_by"],
            ["sms_users.id"],
            name="fk_sms_student_guardian_links_created_by",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["updated_by"],
            ["sms_users.id"],
            name="fk_sms_student_guardian_links_updated_by",
            ondelete="SET NULL",
        ),
        Index(
            "uq_sms_student_guardian_links_active",
            "student_id",
            "guardian_id",
            unique=True,
            postgresql_where=text("status IN ('ACTIVE', 'SUSPENDED') AND effective_until IS NULL"),
        ),
        ...
        Index(
            "ix_sms_student_guardian_links_tenant_verification", "tenant_id", "verification_status"
        ),
    )

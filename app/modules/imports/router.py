...
from app.modules.imports.schemas import (
    PreviewResponse, UploadResponse, ManualAddStudentRequest, ManualAddStudentResponse, 
    AcademicYearLookup, ProgrammeLookup, BatchLookup, SectionLookup, ActivatePortalResponse, 
    BulkActivateSectionRequest, BulkActivateSectionResponse, BulkActivateEligibilityResponse
)
...
def get_academic_years_lookup(
    ...
    return [{"id": getattr(y, "id"), "name": getattr(y, "name")} for y in years]
...
def get_programmes_lookup(
    ...
    return [{"id": getattr(p, "id"), "name": getattr(p, "programme_name")} for p in programmes]
...
def get_batches_lookup(
    ...
    return [{"id": getattr(b, "id"), "name": getattr(b, "batch_name")} for b in batches]
...
def get_sections_lookup(
    ...
    return [{"id": getattr(s, "id"), "name": getattr(s, "section_name")} for s in sections]

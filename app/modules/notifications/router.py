...
def verify_meta_webhook(
    ...
    logger.warning(f"Meta Webhook verification failed for token: {token}")
...
def update_guardian_phone(
    ...
    link = db.execute(
        text("SELECT guardian_id FROM sms_student_guardian_links WHERE student_id = :id AND is_primary = true"),
    ...
    if link and link.guardian_id:
        ...
        gid = uuid.uuid4()
        db.execute(
            text("INSERT INTO sms_guardians (id, full_name, mobile) VALUES (:id, 'Primary Guardian', :mobile)"),
            {"id": gid, "mobile": payload.mobile}
        )
        db.execute(
            text("INSERT INTO sms_student_guardian_links (student_id, guardian_id, is_primary) VALUES (:sid, :gid, true)"),
    ...
    return {"status": "success", "mobile": payload.mobile, "re_dispatched_count": len(pending_logs) if 'pending_logs' in locals() else 0}

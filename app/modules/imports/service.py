...
class ImportService:
    ...
    def upload_student_excel(
        ...
        if existing_batch is not None:
            ...
            if existing_batch.status in {"UPLOADED", "PREVIEW", "FAILED"}:
                return self._revalidate_student_batch(
    ...
    def _revalidate_student_batch(
        ...
        return UploadResponse(
            message="File revalidated with the latest import rules.",
            batch_id=batch.id,
            status=batch.status,
        )
    ...
    def generate_student_import_template(self, tenant_id: UUID, context_branch_id: UUID | None) -> bytes:
        ...
        instructions_rows = [
            ...
            ("Dates", "Use DD/MM/YYYY or DD-MM-YYYY format for Date Of Birth, Joining Date, and Ending Date."),
        ...
        max_data_row = 500
        for column_letter in ("D", "M", "N"):
            for row_number in range(3, max_data_row + 1):
                sheet[f"{column_letter}{row_number}"].number_format = "DD/MM/YYYY"
    ...
    def correct_import_row(
        ...
        return self._revalidate_student_preview_rows(
            batch=batch,
            rows=rows,
            tenant_id=tenant_id,
            context_branch_id=context_branch_id,
    ...
    def correct_import_rows(
        ...
        batch = self.repository.get_batch(batch_id)
        if not batch or batch.tenant_id != tenant_id or batch.module_code != "students":
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Batch not found.")
        if context_branch_id and batch.branch_id and batch.branch_id != context_branch_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized for this batch.")
        if batch.status not in ["PREVIEW", "FAILED"]:
    ...
    def _revalidate_student_preview_rows(
        self,
        batch: ImportBatch,
        rows: list[ImportRow],
        tenant_id: UUID,
        ...
        self.repository.update_batch(batch)
        self.session.commit()
        return self.get_import_preview(batch.id, tenant_id, context_branch_id, expected_module_code="students")
